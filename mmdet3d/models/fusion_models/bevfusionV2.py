...
class BEVFusionV2(Base3DFusionModel):
    def __init__(
        self,
        encoders: Dict[str, Any],
        bevfeats_processor: Dict[str, Any],
        fuser: Dict[str, Any],
        ...
        **kwargs,
        ...
        if bevfeats_processor is not None:
            self.bevfeats_processor = nn.ModuleDict(
                {
                    "backbone": build_backbone(bevfeats_processor["backbone"]) if "backbone" in bevfeats_processor else None,
                    "neck": build_neck(bevfeats_processor["neck"]) if "neck" in bevfeats_processor else None,
                }
            )
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        for sensor in (
            ...
            if sensor == "camera":
                feature, ins_heatmap = self.extract_camera_features(
                    img,
                    points,
                    radar,
                    camera2ego,
            ...
            if self.bevfeats_processor['neck'] is not None:
                feature = self.bevfeats_processor['neck'](feature)
