...
class CenterHead(BaseModule):
    ...
    def _gather_feat(self, feat, ind, mask=None):
        ...
        feat = feat.gather(1, ind.to(torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")))
        if mask is not None:
            ...
            Returns:
                tuple[list[torch.Tensor]]: Tuple of target including \
                    ...
                    - list[torch.Tensor]: Masks indicating which \
                        boxes are valid.
        """
        if isinstance(gt_bboxes_3d, DataContainer):
            gt_bboxes_3d = gt_bboxes_3d.data[0]
        if isinstance(gt_labels_3d, DataContainer):
            gt_labels_3d = gt_labels_3d.data[0]
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            device = preds_dict[0]["heatmap"].device
            loss_heatmap = self.loss_cls(
                preds_dict[0]["heatmap"],
                heatmaps[task_id].to(device),
            ...
            loss_bbox = self.loss_bbox(
                pred, 
                target_box.to(device),
                bbox_weights.to(device),
                avg_factor=(num + 1e-4)
            )
